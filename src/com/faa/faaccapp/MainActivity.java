 ...
 * Data & aksi via engine built-in (root): assets/engine.sh, tanpa module.
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        refreshAppsList();
        updateHome();
        // Deteksi root + siapkan engine built-in di background
        new Thread(new Runnable() {
            @Override
            public void run() {
                final boolean r = RootShell.hasRoot();
                final boolean m = r
                        && FaaccEngine.ensure(MainActivity.this) != null;
    ...
    private LinearLayout buildSettingsPage() {
        ...
        // Interval scheduler = fitur module (faacc service). Versi standalone
        // tanpa module: cleaner jalan on-demand, jadi interval disembunyikan.
        body.addView(c2);
        ...
        c4.addView(infoRow("Engine", "Built-in (tanpa module)"));
    ...
    private void updateNotice() {
        if (!rooted) {
            ...
            homeNotice.setText(
                    "Butuh root untuk scan & clean. Menampilkan daftar aplikasi saja.");
        } else if (!moduleOk) {
            homeNotice.setVisibility(View.VISIBLE);
            homeNotice.setText(
                    "Engine gagal disiapkan. Scan ulang atau restart aplikasi.");
        } else {
            homeNotice.setVisibility(View.GONE);
        }
    }
    ...
    /** Scan cache via engine built-in (root, tanpa module). */
    private void scanCache() {
        if (scanning) {
            return;
        }
        if (!moduleOk && !rooted) {
            toast("Scan butuh root");
            return;
        }
        scanning = true;
        runOnUiThread(new Runnable() {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                // Engine built-in: daftar pkg dari PackageManager (cepat),
                // ukuran dari engine.sh per chunk (progress per chunk).
                final java.io.File bin =
                        FaaccEngine.ensure(MainActivity.this);
                final boolean noEngine = (bin == null);
                final List<AppEntry> list = new ArrayList<AppEntry>();
                final long[] total = {0};
                ...
                if (!noEngine) {
                    try {
                        ...
                        final Map<String, ApplicationInfo> infoMap =
                                new HashMap<String, ApplicationInfo>();
                        for (ApplicationInfo ai : infos) {
                            if (FaaccEngine.validPackage(ai.packageName)) {
                                pkgs.add(ai.packageName);
                                infoMap.put(ai.packageName, ai);
                            }
                        }
                        final int totalPkgs = pkgs.size();
                        Map<String, long[]> sizes =
                                new HashMap<String, long[]>();
                        int done = 0;
                        boolean chunkFail = false;
                        for (int s = 0; s < pkgs.size(); s += 100) {
                            int e = Math.min(s + 100, pkgs.size());
                            Map<String, long[]> m = FaaccEngine.scanChunk(
                        ...
                        if (!chunkFail) {
                            for (ApplicationInfo ai : infos) {
                                AppEntry en = new AppEntry();
                                en.pkg = ai.packageName;
                                try {
                                    en.label = String.valueOf(
                                            pm.getApplicationLabel(ai));
                                } catch (Exception ex) {
                                    en.label = ai.packageName;
                                }
                                long[] v = sizes.get(en.pkg);
                                en.bytes = (v == null) ? 0 : v[0];
                                if (en.bytes > 0) {
                                    withCache[0]++;
                                }
                                total[0] += en.bytes;
                                en.isSystem = (ai.flags
                ...
                final boolean fFailed = failed[0];
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        scanning = false;
                        homeScanBox.setVisibility(View.GONE);
                        if (!fFailed && !list.isEmpty()) {
                            allApps.clear();
                            allApps.addAll(list);
                            totalBytes = total[0];
                            checked.clear();
                            ...
                            toastLong("Scan gagal. Pastikan akses root diberikan.");
                        }
                    }
                });
            }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (final AppEntry e : sel) {
                    ...
                    java.io.File bin =
                            FaaccEngine.ensure(MainActivity.this);
                    long[] cr = (bin == null) ? null
                            : FaaccEngine.cleanPkg(MainActivity.this, bin,
                                    e.pkg);
                    // rc 0 = sukses; 2 = sudah bersih (anggap sukses, 0 freed)
                    boolean ok = (cr != null && cr[1] != 1 && cr[1] != 3);
                    long f = 0;
                    if (ok && cr != null) {
                        f = Math.max(cr[0], 0);
                    } else {
